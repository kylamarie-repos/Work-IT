...
export default function AddField() {
    ...
    const addFieldToDatabase = async () => {
        const fieldValue = Number(fieldName); // Parse the field as a number

        if (isNaN(fieldValue)) {
            setMessage('Field name must be a number');
            return;
        }

        try {
            // Add a new document with an auto-generated ID
            await addDoc(collection(db, 'salaries'), { salary: fieldValue });
    ...
    return (
        <div className="container mt-5">
            <h2>Add New Field</h2>
            <div className="mb-3">
                <label htmlFor="fieldName" className="form-label">Field Name (Number)</label>
                <input
                    type="text"
